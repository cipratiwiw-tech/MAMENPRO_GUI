...
class EditorBinder(QObject):
    ...
    Versi: CLEAN & OPTIMIZED (No Duplicate Connections)
    """
    ...
    def _connect_logic_to_ui(self):
        """Logic -> UI (Controller memerintahkan update tampilan)"""
        # 1. CRUD Visual (Layer Created/Removed)
        ...
        # 4. Jantung Aplikasi (Time & Preview Sync)
        self.c.sig_preview_update.connect(self._on_preview_update)

    def _connect_ui_to_logic(self):
        """UI -> Logic (User melakukan aksi)"""
        
        # 1. TIMELINE & LAYER ACTIONS (Pusat Interaksi)
        # Pastikan panel sudah memiliki sinyal ini (sudah diperbaiki di langkah sebelumnya)
        if hasattr(self.ui.layer_panel, 'sig_request_seek'):
            ...
            self.ui.layer_panel.sig_request_move.connect(self.c.move_layer_time) # Drag & Drop
            
            # Legacy Signals (Tombol Add/Del di panel lama, jika masih ada)
        ...
        # 4. SPECIAL FEATURES (Chroma & Caption)
    ...
    def _on_layer_created(self, layer_data):
        # Refresh total timeline agar layout track benar
    ...
    def _on_layer_removed(self, layer_id):
        self.ui.layer_panel.sync_all_layers(self.c.state.layers) # Refresh total lebih aman
    ...
    def _on_property_changed(self, layer_id, props):
        # HANYA refresh timeline jika properti WAKTU berubah
